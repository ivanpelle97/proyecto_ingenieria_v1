...
/*
 * Firmware de entrenamiento en canal fijo.
 *
 * Objetivo:
 *   - Mantener la antena ESP32 conectada al AP todo el tiempo.
 *   - Escuchar siempre sobre el canal fijo del AP.
 *   - Capturar solo durante los puntos activos de entrenamiento.
 *   - Filtrar por MAC objetivo desde la propia antena para reducir ruido.
 *
 * Uso recomendado:
 *   - Router 2.4 GHz fijo en canal 1.
 *   - TAG emitiendo en el mismo canal.
 *   - Una placa por identificador: A1, A2, A3, A4.
 */
...
static const char *ANCHOR_ID = "A4";
...
#define FLUSH_PACKET_THRESHOLD 12
#define FLUSH_INTERVAL_MS 600
#define CONFIG_REFRESH_MS 1200
#define STATUS_LOG_INTERVAL_MS 5000
#define WIFI_LOOP_DELAY_MS 60
...
static bool should_capture_now() {
...
static void ensure_promiscuous_capture() {
  if (!wifi_has_ip() || !should_capture_now()) {
    if (promisc_enabled) {
      esp_wifi_set_promiscuous(false);
      promisc_enabled = false;
      logp("sniffer: OFF");
...
static bool fetch_server_config() {
  ...
  if (!config_enabled || !capture_active || !target_mac_valid) {
    clear_capture_buffer();
  }

  logp(
    "CFG: enabled=%s session=%s campaign=%s capture_active=%s target=%s target_ok=%s samples=%u",
...
static bool post_snapshot_json(int n) {
  ...
  if (code > 0) {
    logp("POST: ok status=%d packets=%d", code, n);
  } else {
...
void setup() {
  Serial.begin(115200);
  delay(1200);
  Serial.println("=== SKETCH START anchor_sniffer_training_fixed_channel ===");
...
void loop() {
  ...
  const bool should_flush =
    pending >= FLUSH_PACKET_THRESHOLD ||
    (pending > 0 && (now_ms - last_flush_ms >= FLUSH_INTERVAL_MS));
  ...
  if (now_ms - last_status_ms >= STATUS_LOG_INTERVAL_MS) {
    ...
    esp_wifi_get_channel(&primary, &second);
    logp(
      "STAT: wifi=%s promisc=%s channel=%u channel_cfg=%u enabled=%s capture_active=%s target_ok=%s buffered=%d",
      wifi_has_ip() ? "up" : "down",
